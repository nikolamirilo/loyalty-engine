...
@mcp.tool()
async def get_member_balance(member_id: str) -> Dict[str, Any]:
    """Get a member's current points balance."""
    require_scope("read")
    return await api.get(f"/members/{member_id}/balance")
...
async def list_transactions(
    ...
    return await api.get(
        f"/members/{member_id}/transactions", params={"skip": skip, "limit": limit}
    )


@mcp.tool()
async def earn_points(
    member_id: str, points: int, description: Optional[str] = None
) -> Dict[str, Any]:
    ...
    return await api.post(
        f"/members/{member_id}/points/earn", {"points": points, "description": description}
    )
...
async def burn_points(
    ...
    require_scope("write")
    return await api.post(
        f"/members/{member_id}/points/burn", {"points": points, "description": description}
    )
